...
class HrContract(models.Model):
    ...
    fecha_antiguedad = fields.Date(string=_('Fecha de antiguedad'))
    departure_reason = fields.Selection(
        selection=[('voluntario', 'Voluntario'),
                   ('involuntario', 'Involuntario'),
                   ('retiro', 'Retiro'), ],
        string=_('Razón de salida'), default='voluntario'
    )
    departure_description = fields.Text('Motivo de salida')
    ingreso_anual = fields.Float('Ingreso anual', 0.00)
    salario_diario = fields.Float('Salario diario', 0.00)
    ingreso_variable_mensual = fields.Float('Ingreso variable mensual', 0.00)
    variable_marzo = fields.Float('Variable marzo', 0.00)
    variable_abril = fields.Float('Variable abril', 0.00)
    porcentaje_prima_vacacional = fields.Float('Prima vacacional %', 0.00)

    antiguedad = fields.Float('Antiguedad', 0.00)
    dias_de_vacaciones = fields.Integer('Dias de vacaciones')
    aguinaldo = fields.Float('Aguinaldo', 0.00)
    prima_vacacional = fields.Float('Prima Vacacional', 0.00)
    despensa_en_efectivo = fields.Float('Despensa en efectivo', 0.00)
    dias = fields.Float('Dias', 0.00)
    sdi = fields.Float('SDI', 0.00)
    tope_salario = fields.Float('Tope salario', 0.00)
    tope_exento = fields.Float('Tope exento', 0.00)
    exedente = fields.Float('Exedente', 0.00)

    @api.onchange('wage')
    def _calculo_sdi(self):
        if self.wage > 0:

            ingreso_anual = self.wage * 12
            salario_diario = self.wage / 30
            porcentaje_pv = self.porcentaje_prima_vacacional / 100
            id_calculo_nomina = self.tabla_nomina.id

            uma = request.env['tabla.nomina'].search([('id', '=', id_calculo_nomina)], limit=1)
            uma_actual = uma.uma

            periodo_mensual_en_dias = uma.periodo_mensual_en_dias

            uma_mensual = uma_actual * periodo_mensual_en_dias

            if ingreso_anual > uma_mensual:
                result_UMA = uma_mensual
            else:
                result_UMA = ingreso_anual

            dif_dates = date.today() - self.fecha_antiguedad
            antiguedad = dif_dates.days / 365

            dias = request.env['tabla.vacaciones'].search([('form_id', '=', uma.id),
            ...
            despensa_en_efectivo = result_UMA / periodo_mensual_en_dias
            ...
            dias = (self.variable_marzo + self.variable_abril) / total_dias_meses
            ...
            # Exedente
            if tope_exento < result_UMA:
                exedente = result_UMA - tope_exento
            else:
                exedente = 0

            self.ingreso_anual = ingreso_anual
            self.salario_diario = salario_diario
            self.ingreso_variable_mensual = result_UMA
...
class TablaNomina(models.Model):
    ...
    @api.onchange('guarderias_y_gastos_de_prevision_social_patron', 'guarderias_y_gastos_de_prevision_social_trabajador')
...
class CalculosNomina(models.Model):
    ...
    def _calculo_imss(self, id_tabla_nomina, tope_salario):
        ...
        if tope_salario > 0:
            ...
            if tope_salario < (calculos.limite_inv_y_vid_y_cv * calculos.salario_minimo_vigente):
                ...
                IV_trab = ((calculos.limite_inv_y_vid_y_cv * calculos.salario_minimo_vigente) * 15) * (calculos.invalidez_y_vida_trabajador / 100)
                IV_patron = ((calculos.limite_inv_y_vid_y_cv * calculos.salario_minimo_vigente) * 15) * (calculos.invalidez_y_vida_patron / 100)
            ...
            if tope_salario < (calculos.limite_inv_y_vid_y_cv * calculos.salario_minimo_vigente):
                ...
                cesantia_y_vejez_trab = ((calculos.limite_inv_y_vid_y_cv * calculos.salario_minimo_vigente) * 15) * (calculos.cesantia_y_vejez_trabajador / 100)
                cesantia_y_vejez_patron = ((calculos.limite_inv_y_vid_y_cv * calculos.salario_minimo_vigente) * 15) * (calculos.cesantia_y_vejez_patron / 100)
    ...
    def compute_sheet(self):
        ...
        # ISR
        tabla_calculos = request.env['tabla.nomina'].search([('id', '=', self.contract_id.tabla_nomina.id)], limit=1)
        salario_diario = self.contract_id.wage / 30
        ingreso_q1 = salario_diario * 15
        ingreso_q2 = salario_diario * 15
        despensa_en_efectivo_q1 = self.contract_id.ingreso_variable_mensual / 2
        despensa_en_efectivo_q2 = self.contract_id.ingreso_variable_mensual / 2
        base_gravable = ingreso_q1 + ingreso_q2 + despensa_en_efectivo_q1 + despensa_en_efectivo_q2
        isr_mensual = request.env['tabla.isr'].search([('form_id', '=', tabla_calculos.id),
                                                       ('limite_inferior', '<', base_gravable),
                                                       ('limite_superior', '>', base_gravable)])
        ...
        tope_salario = self.contract_id.tope_salario
        ...
        if tope_salario > 0:
            ...
            # PRESTACIONES EN ESPECIE (PE)
            if tope_salario > uma_actual * 3:
                PE_3SMG_trab = ((tope_salario - (uma_actual * 3)) * tabla_calculos.excedente_trabajador / 100) * 15
                PE_3SMG_patron = ((tope_salario - (uma_actual * 3)) * tabla_calculos.excedente_patron / 100) * 15
                PE_3SMG_total = PE_3SMG_trab + PE_3SMG_patron
            ...
            GSP_patron = (tope_salario * 15) * (tabla_calculos.guarderias_y_gastos_de_prevision_social_patron / 100)
            GSP_total = GSP_trab + GSP_patron
            ...
            if tope_salario < (tabla_calculos.limite_inv_y_vid_y_cv * tabla_calculos.salario_minimo_vigente):
                ...
                TI_patron = ((
                                         tabla_calculos.limite_inv_y_vid_y_cv * tabla_calculos.salario_minimo_vigente) * 15) * (tabla_calculos.infonavit_patron / 100)
